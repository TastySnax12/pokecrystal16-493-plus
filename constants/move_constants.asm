...
; Gen 3
	...
	const DOOM_DESIRE  ; 166
	const PSYCHO_BOOST ; 167
; Gen 4
	const ROOST        ; 168
	const GRAVITY      ; 169
	const MIRACLE_EYE  ; 16a
	const WAKE_UP_SLAP ; 16b
	const HAMMER_ARM   ; 16c
	const GYRO_BALL    ; 16d
	const HEALING_WISH ; 16e
	const BRINE        ; 16f
	const NATURAL_GIFT ; 170
	const FEINT        ; 171
	const PLUCK        ; 172
	const TAILWIND     ; 173
	const ACUPRESSURE  ; 174
	const METAL_BURST  ; 175
	const U_TURN       ; 176
	const CLOSE_COMBAT ; 177
	const PAYBACK      ; 178
	const ASSURANCE    ; 179
	const EMBARGO      ; 17a
	const FLING        ; 17b
	const PSYCHO_SHIFT ; 17c
	const TRUMP_CARD   ; 17d
	const HEAL_BLOCK   ; 17e
	const WRING_OUT    ; 17f
	const POWER_TRICK  ; 180
	const GASTRO_ACID  ; 181
	const LUCKY_CHANT  ; 182
	const ME_FIRST     ; 183
	const COPYCAT      ; 184
	const POWER_SWAP   ; 185
	const GUARD_SWAP   ; 186
	const PUNISHMENT   ; 187
	const LAST_RESORT  ; 188
	const WORRY_SEED   ; 189
	const SUCKER_PUNCH ; 18a
	const TOXIC_SPIKES ; 18b
	const HEART_SWAP   ; 18c
	const AQUA_RING    ; 18d
	const MAGNET_RISE  ; 18e
	const FLARE_BLITZ  ; 18f
	const FORCE_PALM   ; 190
	const AURA_SPHERE  ; 191
	const ROCK_POLISH  ; 192
	const POISON_JAB   ; 193
	const DARK_PULSE   ; 194
	const NIGHT_SLASH  ; 195
	const AQUA_TAIL    ; 196
	const SEED_BOMB    ; 197
	const AIR_SLASH    ; 198
	const X_SCISSOR    ; 199
	const BUG_BUZZ     ; 19a
	const DRAGON_PULSE ; 19b
	const DRAGON_RUSH  ; 19c
	const POWER_GEM    ; 19d
	const DRAIN_PUNCH  ; 19e
	const VACUUM_WAVE  ; 19f
	const FOCUS_BLAST  ; 1a0
	const ENERGY_BALL  ; 1a1
	const BRAVE_BIRD   ; 1a2
	const EARTH_POWER  ; 1a3
	const SWITCHEROO   ; 1a4
	const GIGA_IMPACT  ; 1a5
	const NASTY_PLOT   ; 1a6
	const BULLET_PUNCH ; 1a7
	const AVALANCHE    ; 1a8
	const ICE_SHARD    ; 1a9
	const SHADOW_CLAW  ; 1aa
	const THUNDER_FANG ; 1ab
	const ICE_FANG     ; 1ac
	const FIRE_FANG    ; 1ad
	const SHADOW_SNEAK ; 1ae
	const MUD_BOMB     ; 1af
	const PSYCHO_CUT   ; 1b0
	const ZEN_HEADBUTT ; 1b1
	const MIRROR_SHOT  ; 1b2
	const FLASH_CANNON ; 1b3
	const ROCK_CLIMB   ; 1b4
	const DEFOG        ; 1b5
	const TRICK_ROOM   ; 1b6
	const DRACO_METEOR ; 1b7
	const DISCHARGE    ; 1b8
	const LAVA_PLUME   ; 1b9
	const LEAF_STORM   ; 1ba
	const POWER_WHIP   ; 1bb
	const ROCK_WRECKER ; 1bc
	const CROSS_POISON ; 1bd
	const GUNK_SHOT    ; 1be
	const IRON_HEAD    ; 1bf
	const MAGNET_BOMB  ; 1c0
	const STONE_EDGE   ; 1c1
	const CAPTIVATE    ; 1c2
	const STEALTH_ROCK ; 1c3
	const GRASS_KNOT   ; 1c4
	const CHATTER      ; 1c5
	const JUDGMENT     ; 1c6
	const BUG_BITE     ; 1c7
	const CHARGE_BEAM  ; 1c8
	const WOOD_HAMMER  ; 1c9
	const AQUA_JET     ; 1ca
	const ATTACK_ORDER ; 1cb
	const DEFEND_ORDER ; 1cc
	const HEAL_ORDER   ; 1cd
	const HEAD_SMASH   ; 1ce
	const DOUBLE_HIT   ; 1cf
	const ROAR_OF_TIME ; 1d0
	const SPACIAL_REND ; 1d1
	const LUNAR_DANCE  ; 1d2
	const CRUSH_GRIP   ; 1d3
	const MAGMA_STORM  ; 1d4
	const DARK_VOID    ; 1d5
	const SEED_FLARE   ; 1d6
	const OMINOUS_WIND ; 1d7
	const SHADOW_FORCE ; 1d8
